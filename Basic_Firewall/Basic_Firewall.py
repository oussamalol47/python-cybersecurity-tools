...
# Check for BadPacket signature
def is_BadPacket(packet):
...
def packet_callback(packet):
    ...
    if src_ip in blacklist_ips:
      ...
      # Check for BadPacket
    if is_BadPacket(packet):
        print(f"Blocking BadPacket source IP: {src_ip}")
        os.system(f"iptables -A INPUT -s {src_ip} -j DROP")
        log_event(f"Blocking BadPacket IP: {src_ip}")
        return
